import React from 'react'
import Clock from 'react-live-clock';

function Homeproducts() {
    // const [time, setTime] = useState('')
    const currTime = new Date().toLocaleTimeString();
    // setTime(currTime)
    return (
        <>
            <div className="container-fluid">
                {/* <div className="orangebox"></div> */}
                <p className="section2head">Todays</p>
                <div className="row">
                    <div className="col-4">
                        <h1 className="flashsalestime">Flash Sales</h1>
                    </div>
                    <div className="col-4">
                        {/* <h1>03:23:19:56</h1> */}
                        <h1><Clock format={'HH:mm:ss'} ticking={true} timezone={'Asia/Colombo'} /></h1>
                    </div>
                </div>

            </div>

            <div className="container-fluid productscontainer">
                <div className="row">
                    <div className="col-md-3">
                        <div class="card">

                            <div class="image-container">

                                <div class="first">

                                    <div class="d-flex justify-content-between align-items-center">

                                        <span class="discount">-25%</span>
                                        <span class="wishlist"><i class="fa fa-heart-o"></i></span>


                                    </div>
                                </div>

                                <img src="https://i.imgur.com/8JIWpnw.jpg" class="img-fluid rounded thumbnail-image" />


                            </div>


                            <div class="product-detail-container p-2">

                                <div class="d-flex justify-content-between align-items-center">

                                    <h5 class="dress-name">White traditional long dress</h5>

                                    <div class="d-flex flex-column mb-2">

                                        <span class="new-price">$3.99</span>
                                        <small class="old-price text-right">$5.99</small>
                                    </div>

                                </div>


                                <div class="d-flex justify-content-between align-items-center pt-1">

                                    <div class="color-select d-flex ">

                                        <input type="button" name="grey" class="btn creme productbtn" />
                                        <input type="button" name="red" class="btn red ml-2 productbtn" />
                                        <input type="button" name="blue" class="btn blue ml-2 productbtn" />

                                    </div>

                                    <div class="d-flex ">

                                        <span class="item-size mr-2 productbtn" type="button">S</span>
                                        <span class="item-size mr-2 productbtn" type="button">M</span>
                                        <span class="item-size productbtn" type="button">L</span>

                                    </div>


                                </div>


                                <div class="d-flex justify-content-between align-items-center pt-1">
                                    <div>
                                        <i class="fa fa-star-o rating-star"></i>
                                        <span class="rating-number">4.8</span>
                                    </div>

                                    <span class="buy">BUY+</span>

                                </div>



                            </div>

                        </div>
                    </div>
                    <div className="col-md-3">
                        <div class="card">

                            <div class="image-container">

                                <div class="first">

                                    <div class="d-flex justify-content-between align-items-center">

                                        <span class="discount">-25%</span>
                                        <span class="wishlist"><i class="fa fa-heart-o"></i></span>


                                    </div>
                                </div>

                                <img src="https://i.imgur.com/8JIWpnw.jpg" class="img-fluid rounded thumbnail-image" />


                            </div>


                            <div class="product-detail-container p-2">

                                <div class="d-flex justify-content-between align-items-center">

                                    <h5 class="dress-name">White traditional long dress</h5>

                                    <div class="d-flex flex-column mb-2">

                                        <span class="new-price">$3.99</span>
                                        <small class="old-price text-right">$5.99</small>
                                    </div>

                                </div>


                                <div class="d-flex justify-content-between align-items-center pt-1">

                                    <div class="color-select d-flex ">

                                        <input type="button" name="grey" class="btn creme productbtn" />
                                        <input type="button" name="red" class="btn red ml-2 productbtn" />
                                        <input type="button" name="blue" class="btn blue ml-2 productbtn" />

                                    </div>

                                    <div class="d-flex ">

                                        <span class="item-size mr-2 productbtn" type="button">S</span>
                                        <span class="item-size mr-2 productbtn" type="button">M</span>
                                        <span class="item-size productbtn" type="button">L</span>

                                    </div>


                                </div>


                                <div class="d-flex justify-content-between align-items-center pt-1">
                                    <div>
                                        <i class="fa fa-star-o rating-star"></i>
                                        <span class="rating-number">4.8</span>
                                    </div>

                                    <span class="buy">BUY +</span>

                                </div>



                            </div>

                        </div>
                    </div>
                </div>
               
            </div>
            <div className="row buttonrow">
            <div className="mb-3">
              <button type="button" class="btn btn-primary morebutton">View All Products</button>
            </div>
            </div>
           
        </>
    )
}

export default Homeproducts